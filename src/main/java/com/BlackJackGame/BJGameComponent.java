...
public class BJGameComponent extends JComponent implements MouseListener { //this class will implement the MouseListener because we will check if the user clicked a certain coordinate on the component.
    ...
    private ArrayList<Card> dealerHand; //we have to card arraylists which will serve as hands: one for the dealer and for the player.
    ...
    public BJGameComponent(ArrayList<Card> dH, ArrayList<Card> pH) //this will be the constructor for this class which will accept two hands as a parameter.
    {
        //Initialize the instant fields.
        dealerHand = dH;
        playerHand = pH;
        dealerScore = 0;
        playerScore = 0;
    ...
    public void paintComponent(Graphics g)
    {
        ...
        try // This is our blackjack table background
        {
            backgroundImage = ImageIO.read(new File("C:/project-1-initial-main/BlackJackGame/src/main/java/com/BlackJackGame/images/Background.png")); //we read a file which is the png image of a poker table for our background image.
            chip = ImageIO.read(new File("C:/project-1-initial-main/BlackJackGame/src/main/java/com/BlackJackGame/images/chip.png"));
        }
...
public void refresh(int cB, int uS, int dS, boolean fD) //this refresh method will refresh the GameComponent when it is called.
{
    ...
    public void mousePressed(MouseEvent e)
    {
        ...
        if(mouseX>= 50 && mouseX<=200 && mouseY>=300 && mouseY<=450) {//we will only do something if the x and y coordinates fall on top of the chip image. The coordinates you see below give the end points of the chip image.
            ...
            BlackJackDriver.newGame.startGame(); //then we start the game.
        }
